...
import Foundation
import SwiftUI
import Supabase                  // supabase library
...
struct SignUpView: View {
    ...
    // MARK: - UI State
    @State private var isLoading = false          // Shows spinner on buttonn
    ...
    // MARK: - Environment
    @Environment(\.dismiss) private var dismiss   //go back to WelcomeView
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 32) {
                ...
                .padding(.horizontal, 32)
                .disabled(isLoading || !isFormValid)          // Disable if input invalid
    ...
    //Determines if form is valid: all fields filled and passwords match with confirm password
    ...
    private func performSignUp() async {
        ...
        do {
            ...
            // Save the user's email locally (for displaying in Settings)
            UserDefaults.standard.set(cleanEmail, forKey: "userEmail")
            
            print("✅ Sign up successful for: \(cleanEmail)")
            print("User ID: \(authResponse.user.id)")  // for debugging
            
            // Navigate to the main app
            isSignedUp = true
            
        } catch let authError as AuthError {
            //  catch specific Supabase auth errors with nice messages
